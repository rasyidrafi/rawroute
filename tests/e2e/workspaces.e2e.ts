...
test("switches dashboard apps with contextual navigation", async ({ page }) => {
  ...
  await expect(sidebar.getByRole("button", { name: "Switch app" })).toHaveCount(0)
  ...
  const workspaceTrigger = sidebar.getByRole("button", { name: /RawRoute/ })
  await expect(workspaceTrigger).toContainText("Default")
  await expect(workspaceTrigger).toContainText("•")
  await expect(workspaceTrigger).toContainText("AI Gateway")
  ...
  const appMenu = page.locator('[data-slot="dropdown-menu-content"]').filter({ hasText: "Workspaces" })
  await expect(appMenu.getByText("Workspaces", { exact: true })).toBeVisible()
  await expect(appMenu.getByText("Apps", { exact: true })).toBeVisible()
  await expect(appMenu.getByRole("menuitemradio", { name: "Default", exact: true })).toHaveAttribute("aria-checked", "true")
  await expect(appMenu.getByRole("menuitemradio", { name: "AI Gateway", exact: true })).toHaveAttribute("aria-checked", "true")
  await expect(appMenu.getByRole("menuitemradio", { name: "Tool Gateway", exact: true })).toBeVisible()
  await expect(appMenu).not.toContainText("(Executor)")
...
test("preserves dashboard app URL state, workspace selection, and mobile navigation", async ({ page }) => {
  ...
  try {
    ...
    await sidebarTrigger.click()
    await expect(mobileSidebar).toBeVisible()
    await mobileSidebar.getByRole("button", { name: /RawRoute/ }).click()
