...
function isWithinMigrationWindow(document: MigratedDocument, prefix: string) {
  const collectionPath = document.path.slice(0, document.path.lastIndexOf("/"))
  ...
  const isRollupCollection = collectionPath === `${prefix}_usage_rollups` || collectionPath.endsWith("/usageRollups")
  if (isRollupCollection) {
...
async function migrateFirestore() {
  ...
  let source = sourceAll.filter((document) => isWithinMigrationWindow(document, prefix))
  ...
  const compare = (sourceDocuments: MigratedDocument[], targetDocuments: Array<{ path: string; data: Record<string, unknown> }>) => {
    ...
    const extra = targetDocuments.filter((document) => !sourcePaths.has(document.path) && !isDerivedApiKeyIndex(document.path, prefix)).length
  ...
  if (!verifyOnly && !dryRun) {
    ...
    for (let pass = 1; pass <= 3; pass += 1) {
      ...
      const stalePaths = targetBeforePass.map((document) => document.path).filter((path) => !sourcePaths.has(path) && !isDerivedApiKeyIndex(path, prefix))
      ...
      source = sourceAll.filter((document) => isWithinMigrationWindow(document, prefix))
  ...
  console.log(JSON.stringify({
    storage: "postgres",
    migrationSince,
    sourceDocumentsBeforeCutoff: initialSourceAll.length,
    sourceDocuments: source.length,
