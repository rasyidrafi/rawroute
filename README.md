...
Grant the Cloud Run service account Firestore access. `roles/datastore.user` is the usual starting point; use a narrower custom role when practical. The proxy caches its routing snapshot in each instance for `ROUTING_CACHE_TTL_MS` (60 seconds by default), avoiding a Firestore read for every inference request. After that TTL, an unchanged instance normally reads only the workspace routing-revision document and reuses its snapshot. Configuration mutations advance the revision, while `ROUTING_FULL_REFRESH_INTERVAL_MS` (15 minutes by default) provides a compatibility safety reload. Admin mutations invalidate the current process immediately; other instances observe the revision at their next cache check.
...
RawRoute is designed so the proxy hot path does not reread the whole Firestore configuration. Routing snapshots, workspace status, gateway-key indexes, budget configuration, pricing, Codex quota snapshots, and assembled dashboard payloads use bounded process-local caches with single-flight loading. A warm gateway-key authentication normally performs no Firestore read. A cold lookup normally reads only the global key index because new index rows carry the authentication metadata; legacy index rows self-heal with one additional owning-key read. An unknown key stops after the index read and is negatively cached instead of scanning every workspace. Expired routing snapshots validate against one small per-workspace revision document and reload providers, credentials, models, aliases, and metadata only when configuration changed or the safety refresh is due.

Existing multi-workspace installations should populate the global gateway-key index once before deploying this behavior:

```bash
npm run firestore:backfill-api-key-indexes -- --dry-run
npm run firestore:backfill-api-key-indexes
```

The command reports counts only, creates or repairs changed index rows, removes orphaned rows, and aborts on duplicate key values. `API_KEY_INDEX_REPAIR_ON_MISS=true` restores cross-workspace repair for a temporary migration window, but it should not remain enabled: arbitrary invalid credentials could otherwise trigger many billed reads.

Upstash Redis performs one atomic reservation and one atomic release/settlement for a normal routed request. RPM buckets, concurrency leases, and cooldown state for all credentials of a provider are compacted into one provider-scoped hash, so the number of Redis keys and commands no longer grows with credential count. Session affinity remains model-specific. Affinity TTL is assignment-based by default, avoiding a write on every sticky hit; set `ROUTING_REFRESH_AFFINITY_TTL=true` only when sliding expiration is required. Responses API mappings with multiple IDs use a pipeline because those independent writes do not need transaction semantics, while admission and settlement remain atomic Lua operations. The pipeline reduces HTTP round trips; its individual Redis commands are still billable, so response IDs are deduplicated and bounded.

The compact state format uses the default `ROUTING_STATE_PREFIX=rawroute:routing:v2`, and budget reservations use a separate v2 key format. Treat a v1-to-v2 rollout as a state cutover: affinity, response mappings, transient capacity, and in-flight budget reservations begin fresh. Old and new application versions must not serve traffic concurrently for an extended period because their state is isolated. Drain the old revision promptly or deploy during a low-traffic window.

Budget admission does not rescan historical usage on every request. RawRoute reconciles Firestore rollups with the current counter, persists the stable offset in the budget document, and reuses it across cold starts and instances for the rest of the matching budget/window revision. Persistence waits for the budget-cache TTL plus the maximum request deadline, preventing a late request admitted under the previous configuration from being permanently omitted. Budget/window revisions are also included in the Redis key, so budget edits, Unlimited Mode transitions, and repricing do not continue using stale reservations. `BUDGET_DEFAULT_OUTPUT_TOKENS`, `BUDGET_INPUT_BYTES_PER_TOKEN`, and `BUDGET_RESERVATION_SAFETY_PERCENT` tune conservative reservations for requests without a strict output cap.

By default, each completed request is persisted atomically as one usage event plus hourly and daily rollups, and one budget counter update only when a budget exists. That is three Firestore writes without a budget and four with one: 25% and 20% fewer writes respectively than also persisting a monthly rollup. Daily rows keep long-range dashboard reads bounded. Set `USAGE_ROLLUP_GRANULARITIES=hourly,daily,monthly` only when measured long-range dashboard traffic makes one extra write per completed request cheaper than reading daily rows. Enabling monthly persistence affects new or explicitly backfilled data; do not switch monthly dashboards to it until historical months are complete. Dashboard queries merge adjacent fallback ranges, reuse boundary reads, bound Firestore fan-out, and briefly cache the finished payload.

Useful controls are documented in `.env.example`. `FIRESTORE_CHILD_READ_CONCURRENCY` and `FIRESTORE_ANALYTICS_READ_CONCURRENCY` cap query fan-out; lower them when protecting a small Firestore quota or low-memory instance. `MAX_WORKSPACE_CACHE_ENTRIES` and `MAX_PROVIDER_SCOPED_CACHE_ENTRIES` bound long-lived multi-tenant caches. Higher cache TTLs generally reduce Firestore and Redis-adjacent work but delay cross-instance visibility. Setting TTLs near zero in production can turn ordinary navigation and proxy traffic into avoidable billable reads.
