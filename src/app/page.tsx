...
export default async function Home({ searchParams }: { searchParams: Promise<{ workspace?: string | string[] }> }) {
  const [allWorkspaces, authenticated] = await Promise.all([listWorkspaces(), isAuthenticated()])
  const workspaces = allWorkspaces.filter((workspace) => workspace.status === "active")
  ...
  return <>
    <header className="border-b bg-background/90 px-4 py-3">
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        ...
        <div className="flex items-center gap-3"><PublicWorkspaceSelector workspaces={workspaces.map(({ id, name }) => ({ id, name }))} workspaceId={workspace.id} /><Link className="text-sm font-medium underline-offset-4 hover:underline" href={authenticated ? "/dashboard" : "/login"}>{authenticated ? "Open dashboard" : "Admin login"}</Link></div>
      </div>
    </header>
