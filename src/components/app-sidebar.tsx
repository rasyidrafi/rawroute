...
import { ArrowLeftRightIcon, KeyRoundIcon, LinkIcon, LogsIcon, LogOutIcon, RouteIcon, ServerIcon, SettingsIcon } from "lucide-react"
...
const navigation = [
  ...
  { title: "OAuth Providers", icon: LinkIcon, href: "/dashboard/oauth-providers" },
    ...
    <Sidebar collapsible="icon" {...props}>
      ...
      <SidebarContent>
        <SidebarGroup><SidebarGroupLabel>Gateway</SidebarGroupLabel><SidebarGroupContent><SidebarMenu>
          {navigation.map((item) => <SidebarMenuItem key={item.href}><SidebarMenuButton isActive={pathname === item.href || (item.href !== "/dashboard" && pathname.startsWith(`${item.href}/`))} tooltip={item.title} render={<Link href={item.href} prefetch={false} />}><item.icon /><span>{item.title}</span></SidebarMenuButton></SidebarMenuItem>)}
