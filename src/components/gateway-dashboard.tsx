...
import { ArrowLeftIcon, BoxesIcon, ChevronRightIcon, CopyIcon, KeyRoundIcon, LockKeyholeIcon, PencilIcon, PlusIcon, RouteIcon, Trash2Icon } from "lucide-react"
...
export function GatewayDashboard({ view, providerId }: { view: "endpoint-key" | "providers" | "provider-detail" | "settings"; providerId?: string }) {
  ...
  const detailProvider = view === "provider-detail" ? state.providers.find((provider) => provider.id === providerId) : undefined
  ...
  return <main className="flex-1 bg-[#f6f5f1] p-4 dark:bg-background md:p-6 lg:p-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-8">
        {view === "providers" && <ProviderList
          ...
          onOpenProvider={(providerId) => router.push(`/dashboard/providers/${providerId}`)}
        ...
        {view === "provider-detail" && <ProviderDetail
          ...
          models={state.models.filter((model) => model.providerId === providerId)}
          isPending={isPending}
          onAddKey={() => { setEditingProviderApiKey(null); setProviderKeyOpen(true) }}
          onAddModel={() => { setEditingModel(null); setModelOpen(true) }}
          ...
          onEditModel={(model) => { setEditingModel(model); setModelOpen(true) }}
          ...
          onDeleteModel={(model) => run({ action: "delete-model", id: model.id }, "Model deleted", `delete-model:${model.id}`)}
        />}
        ...
        <Dialog open={modelOpen} onOpenChange={(open) => { setModelOpen(open); if (!open) setEditingModel(null) }}><DialogContent>{detailProvider && <ModelForm key={editingModel?.id || "new"} provider={detailProvider} model={editingModel} onSave={async (model) => { const saved = await run({ action: "save-model", model }, editingModel ? "Model updated" : "Model saved", "save-model"); if (saved) setModelOpen(false); return saved }} />}</DialogContent></Dialog>
...
function ProviderList({ providers, providerApiKeys, models, isPending, onAdd, onOpenProvider, onDeleteProviderList }: { providers: Provider[]; providerApiKeys: ProviderApiKey[]; models: Model[]; isPending: (key: string) => boolean; onAdd: () => void; onOpenProvider: (providerId: string) => void; onDeleteProviderList: (provider: Provider) => Promise<boolean> }) {
  return <Card><CardHeader><CardTitle>Providers</CardTitle><CardDescription>Choose a provider to manage its connection settings and upstream API keys.</CardDescription><CardAction><Button onClick={onAdd}><PlusIcon />Add provider</Button></CardAction></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Provider</TableHead><TableHead>Prefix</TableHead><TableHead>Protocol</TableHead><TableHead>Origin</TableHead><TableHead>API keys</TableHead><TableHead>Models</TableHead><TableHead /></TableRow></TableHeader><TableBody>{providers.map((provider) => {
    ...
    const enabledModelCount = models.filter((model) => model.providerId === provider.id && model.enabled).length
    return <TableRow key={provider.id} className="cursor-pointer" onClick={() => onOpenProvider(provider.id)}><TableCell><Link href={`/dashboard/providers/${provider.id}`} className="font-medium hover:underline" onClick={(event) => event.stopPropagation()}>{provider.name}</Link></TableCell><TableCell><Badge variant="secondary">{provider.prefix}/</Badge></TableCell><TableCell>{protocolLabels[provider.protocol]}</TableCell><TableCell className="max-w-64 truncate font-mono text-xs">{provider.baseUrl}</TableCell><TableCell><div className="flex items-center gap-2"><span className="font-medium tabular-nums">{counts.configured}</span><span className="text-xs text-muted-foreground">configured</span>{provider.authType !== "none" && counts.enabled !== counts.configured && <Badge variant="outline">{counts.enabled} enabled</Badge>}</div></TableCell><TableCell><div className="flex items-center gap-2"><span className="font-medium tabular-nums">{modelCount}</span><span className="text-xs text-muted-foreground">configured</span>{enabledModelCount !== modelCount && <Badge variant="outline">{enabledModelCount} enabled</Badge>}</div></TableCell><TableCell onClick={(event) => event.stopPropagation()}><div className="flex justify-end gap-1"><ConfirmAction title={`Delete ${provider.name}?`} description={`This permanently deletes ${counts.configured} API keys and ${modelCount} models attached to this provider.`} pending={isPending(`delete-provider:${provider.id}`)} onConfirm={() => onDeleteProviderList(provider)}><Trash2Icon /></ConfirmAction><Button nativeButton={false} aria-label={`Open ${provider.name}`} size="icon-sm" variant="ghost" render={<Link href={`/dashboard/providers/${provider.id}`} />}><ChevronRightIcon /></Button></div></TableCell></TableRow>
  })}{!providers.length && <EmptyRow label="No providers yet." colSpan={7} />}</TableBody></Table></CardContent></Card>
}

function ProviderDetail({ provider, apiKeys, models, isPending, onAddKey, onAddModel, onEditProvider, onEditKey, onEditModel, onDeleteProvider, onDeleteKey, onDeleteModel }: {
  provider?: Provider
  ...
  onDeleteModel: (model: Model) => Promise<boolean>
}) {
  ...
  return <>
    <div>
      ...
      <div><h2 className="text-2xl font-semibold tracking-tight">{provider.name}</h2><p className="mt-1 text-sm text-muted-foreground">{counts.configured} API {counts.configured === 1 ? "key" : "keys"} configured</p></div>
    </div>
    <Card>
      <CardHeader>
        <CardTitle>Provider details</CardTitle>
        <CardDescription><span className="font-mono">{provider.baseUrl}</span></CardDescription>
        <CardAction><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => onEditProvider(provider)}><PencilIcon />Edit</Button><ConfirmAction buttonLabel="Delete provider" title={`Delete ${provider.name}?`} description={`This permanently deletes ${counts.configured} API keys and ${models.length} models attached to this provider.`} pending={isPending(`delete-provider:${provider.id}`)} onConfirm={() => onDeleteProvider(provider)}><Trash2Icon /></ConfirmAction></div></CardAction>
      </CardHeader>
      <CardContent><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><DetailValue label="Gateway prefix" value={`${provider.prefix}/`} mono /><DetailValue label="Authentication" value={provider.authType === "none" ? "None" : provider.authType} /><DetailValue label="Protocol" value={protocolLabels[provider.protocol]} /><DetailValue label="Configured keys" value={String(counts.configured)} /><DetailValue label="Configured models" value={String(models.length)} /></div></CardContent>
    </Card>
    <Card>
      <CardHeader>
        ...
        <CardAction><Button disabled={provider.authType === "none"} onClick={onAddKey}><PlusIcon />Add API key</Button></CardAction>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Limits</TableHead><TableHead>Status</TableHead><TableHead>Created</TableHead><TableHead /></TableRow></TableHeader>
          <TableBody>{apiKeys.map((apiKey) => { const pendingKey = `delete-provider-api-key:${apiKey.id}`; return <TableRow key={apiKey.id}><TableCell className="font-medium">{apiKey.name}</TableCell><TableCell><div className="flex flex-wrap items-center gap-1 text-xs text-muted-foreground">{apiKey.rpmLimit && <Badge variant="outline">{apiKey.rpmLimit} rpm</Badge>}{apiKey.maxConcurrency && <Badge variant="outline">{apiKey.maxConcurrency} conc</Badge>}{apiKey.priority ? <Badge variant="outline">prio {apiKey.priority}</Badge> : <Badge variant="outline">prio 0</Badge>}</div></TableCell><TableCell><Badge variant={apiKey.enabled ? "secondary" : "outline"}>{apiKey.enabled ? "Enabled" : "Disabled"}</Badge></TableCell><TableCell className="text-xs text-muted-foreground">{new Date(apiKey.createdAt).toLocaleDateString()}</TableCell><TableCell><div className="flex justify-end gap-1"><Button aria-label={`Edit ${apiKey.name}`} size="icon-sm" variant="ghost" onClick={() => onEditKey(apiKey)}><PencilIcon /></Button><ConfirmAction title={`Delete ${apiKey.name}?`} description="Requests currently routed through this key will fail." pending={isPending(pendingKey)} onConfirm={() => onDeleteKey(apiKey)}><Trash2Icon /></ConfirmAction></div></TableCell></TableRow> })}{!apiKeys.length && <EmptyRow label={provider.authType === "none" ? "This provider does not require API keys." : "No API keys yet."} colSpan={5} />}</TableBody>
        </Table>
      </CardContent>
    </Card>
    <Card>
      <CardHeader>
        ...
        <CardAction><Button onClick={onAddModel}><PlusIcon />Add model</Button></CardAction>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader><TableRow><TableHead>Model</TableHead><TableHead>Gateway ID</TableHead><TableHead>Upstream model</TableHead><TableHead>Protocol</TableHead><TableHead /></TableRow></TableHeader>
          <TableBody>{models.map((model) => { const pendingKey = `delete-model:${model.id}`; return <TableRow key={model.id}><TableCell className="font-medium">{model.name}</TableCell><TableCell><div className="flex items-center justify-between gap-2"><div className="min-w-0 font-mono text-xs font-medium"><span className="break-all">{model.id}</span></div><Button aria-label={`Copy gateway ID ${model.id}`} size="icon-sm" variant="outline" className="shrink-0" onClick={() => { void navigator.clipboard.writeText(model.id); toast.success("Gateway ID copied") }}><CopyIcon /></Button></div></TableCell><TableCell>{model.upstreamModel}</TableCell><TableCell>{protocolLabels[model.protocol || provider.protocol]}{model.protocol && <Badge variant="outline" className="ml-2">override</Badge>}</TableCell><TableCell><div className="flex justify-end gap-1"><Button aria-label={`Edit ${model.name}`} size="icon-sm" variant="ghost" onClick={() => onEditModel(model)}><PencilIcon /></Button><ConfirmAction title={`Delete ${model.name}?`} description={`This permanently removes the ${model.name} mapping.`} pending={isPending(pendingKey)} onConfirm={() => onDeleteModel(model)}><Trash2Icon /></ConfirmAction></div></TableCell></TableRow> })}{!models.length && <EmptyRow label="No models yet." colSpan={5} />}</TableBody>
        </Table>
      </CardContent>
    </Card>
  </>
