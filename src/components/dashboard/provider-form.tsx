...
export function ProviderForm({ provider, onSave }: { provider: Provider | null; onSave: (provider: Partial<Provider> & { originalId?: string }) => Promise<boolean> }) {
  ...
  return <form onSubmit={submit}><DialogHeader><DialogTitle>{provider ? "Edit provider" : "Add provider"}</DialogTitle><DialogDescription>Configure the upstream origin first, then attach one or more API keys. RawRoute keeps the provider catalog and CLIProxy handles translation and forwarding.</DialogDescription></DialogHeader><div className="grid gap-4 py-4"><FormField label="Name"><Input name="name" defaultValue={provider?.name} placeholder="OpenAI" required /></FormField><div className="grid gap-4 sm:grid-cols-2"><FormField label="Prefix"><Input name="prefix" defaultValue={provider?.prefix} placeholder="oa" required /></FormField><FormField label="Default protocol"><Select value={protocol} onValueChange={(value) => { if (value) setProtocolAndAuth(value as Protocol) }} itemToStringLabel={(value) => protocolLabels[value as Protocol] || String(value)}><SelectTrigger className="h-9 w-full"><SelectValue /></SelectTrigger><SelectContent>{protocols.map((item) => <SelectItem key={item} value={item}>{protocolLabels[item]}</SelectItem>)}</SelectContent></Select></FormField></div><FormField label="Base URL"><Input name="baseUrl" value={baseUrl} onChange={(event) => setBaseUrlAndAuth(event.target.value)} type="url" placeholder={protocol === "anthropic-messages" ? "https://api.anthropic.com" : "https://api.openai.com/v1"} required /></FormField><FormField label="Authentication"><Select value={authType} onValueChange={(value) => { if (value) setAuthType(value as AuthType) }} itemToStringLabel={(value) => authLabels[value as AuthType] || String(value)}><SelectTrigger className="h-9 w-full"><SelectValue /></SelectTrigger><SelectContent>{authOptions.map((item) => <SelectItem key={item} value={item}>{authLabels[item]}</SelectItem>)}</SelectContent></Select></FormField>{protocol === "anthropic-messages" && authOptions.includes("x-api-key") ? <p className="-mt-2 text-xs text-muted-foreground">CLIProxy appends /v1/messages. You may enter the Anthropic URL with or without a trailing /v1; RawRoute stores the canonical base URL. x-api-key is available only for the first-party Anthropic endpoint.</p> : protocol !== "anthropic-messages" ? <p className="-mt-2 text-xs text-muted-foreground">CLIProxy sends provider credentials as Bearer tokens. Static headers below are forwarded separately.</p> : null}<FormField label="Static headers (JSON)"><Textarea name="headers" defaultValue={JSON.stringify(provider?.headers || {}, null, 2)} className="font-mono text-xs" /></FormField></div><DialogFooter><FormSubmitButton pending={pending} idleLabel={provider ? "Update provider" : "Save provider"} pendingLabel={provider ? "Updating provider..." : "Saving provider..."} /></DialogFooter></form>
}
