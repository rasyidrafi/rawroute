...
import { CheckIcon, AlertTriangleIcon, CalendarDaysIcon, ChevronsUpDownIcon, Clock3Icon, DollarSignIcon, Link2Icon, PencilIcon, PlusIcon, RefreshCwIcon, RotateCcwIcon, SparklesIcon, WalletCardsIcon } from "lucide-react"
...
type BudgetEntry = { apiKeyId: string; name: string; weeklyLimitMicros: number; spentMicros: number; enabled: boolean; lastUsedAt?: string | null }
...
export function ModelPricingView() {
  ...
  return <Panel title="Model pricing" description="Group compatible gateway models, version their rates, and keep historical usage tied to the price that was active at request time." icon={<DollarSignIcon />} refresh={() => void refreshGroups()} loading={isValidating || pending}>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Popover open={canonicalPopoverOpen} onOpenChange={(open) => { setCanonicalPopoverOpen(open); if (open) { setCanonicalLoading(true); setCanonicalError(null); setCanonicalDebouncedSearch(canonicalSearch.trim()) } }}>
                  ...
                  <div className="flex h-[min(18rem,var(--available-height))] min-h-0">
                    <Command className="min-h-0 flex-1 rounded-none p-0" shouldFilter={false}>
                      ...
                      <CommandList className="max-h-none min-h-0 flex flex-1 flex-col overflow-hidden px-2 py-1">
                        ...
                        {canonicalLoading ? <div className="space-y-2 p-3">{Array.from({ length: 5 }, (_, index) => <div key={index} className="flex items-center gap-3 rounded-md px-2 py-2"><div className="min-w-0 flex-1 space-y-2"><div className="h-4 w-40 animate-pulse rounded bg-muted" /><div className="h-3 w-56 max-w-full animate-pulse rounded bg-muted" /></div><div className="h-5 w-24 animate-pulse rounded bg-muted" /></div>)}</div> : canonicalError ? <CommandEmpty>{canonicalError}</CommandEmpty> : canonicalModels.length === 0 ? <CommandEmpty>{canonicalSearch.trim() ? `No canonical models matched "${canonicalSearch.trim()}".` : "No canonical models available."}</CommandEmpty> : <CommandGroup heading={`Canonical models (${canonicalModels.length})`} className="min-h-0 flex flex-1 flex-col overflow-hidden p-0"><LegendList<CanonicalModelSummary> data={canonicalModels} keyExtractor={(model) => model.id} estimatedItemSize={70} className="h-full min-h-0 overflow-y-auto overscroll-y-contain px-1 py-1 outline-none [&>div]:!block [&>div]:!min-w-0 [&>div]:!w-full" renderItem={({ item }) => <CommandItem value={`${item.name} ${item.id} ${item.provider}`} className="items-start gap-3 rounded-md px-3 py-2" onSelect={() => { setCanonicalModelId(item.id); setCanonicalModel(item); setCanonicalSearch(""); setCanonicalPopoverOpen(false) }}><div className="min-w-0 flex-1"><div className="truncate font-medium">{item.name}</div><div className="truncate text-xs text-muted-foreground">{item.id} · {item.provider}</div><div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-muted-foreground"><span>Input {formatCanonicalRate(item.pricing.inputMicrosPerMillion)}</span><span>Output {formatCanonicalRate(item.pricing.outputMicrosPerMillion)}</span><span>Cache read {formatCanonicalRate(item.pricing.cacheReadMicrosPerMillion)}</span></div></div>{canonicalModelId === item.id && <CheckIcon className="mt-1 size-4 text-primary" />}</CommandItem>} /></CommandGroup>}
                      </CommandList>
                    </Command>
                  </div>
                  </PopoverContent>
    ...
    <Dialog open={replaceOpen} onOpenChange={setReplaceOpen}><DialogContent><DialogHeader><DialogTitle>Reprice historical usage?</DialogTitle><DialogDescription>This replaces the active version and recalculates historical usage for this group in a background job. Existing totals may change.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setReplaceOpen(false)} disabled={pending}>Cancel</Button><Button variant="destructive" onClick={() => void saveVersion("replace")} disabled={pending}>{pending && <LoadingSpinner />}Replace and reprice</Button></DialogFooter></DialogContent></Dialog>
  </Panel>
}
