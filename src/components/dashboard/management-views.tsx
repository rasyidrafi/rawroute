...
export function ModelPricingView() {
  ...
  function handleCanonicalListWheel(event: WheelEvent<HTMLDivElement>) {
    event.preventDefault()
    event.stopPropagation()
    event.currentTarget.scrollBy({ top: event.deltaY, left: event.deltaX })
  }
                ...
                <Popover open={canonicalPopoverOpen} onOpenChange={(open) => { setCanonicalPopoverOpen(open); if (open) { setCanonicalLoading(true); setCanonicalError(null); setCanonicalDebouncedSearch(canonicalSearch.trim()) } }}>
                  ...
                  <div className="flex h-[min(18rem,var(--available-height))] min-h-0">
                    <Command className="min-h-0 flex-1 rounded-none p-0" shouldFilter={false}>
                      ...
                      <CommandList className="max-h-none min-h-0 flex flex-1 flex-col overscroll-contain overflow-y-auto px-2 py-1" onWheel={handleCanonicalListWheel}>
