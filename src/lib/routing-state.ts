...
local responseAffinity = responseIndex > 0 and redis.call("GET", KEYS[responseIndex]) or false
if hard and responseIndex > 0 and required == "" and not responseAffinity then
  return {"hard-missing"}
end
local sessionCredential = affinityIndex > 0 and redis.call("GET", affinity) or false
...
if budgetKeyIndex > 0 then
  local budgetOffset = 9 + (count * 4)
  local limit = tonumber(ARGV[budgetOffset + 1])
  local reservation = tonumber(ARGV[budgetOffset + 2])
  local initialSpent = tonumber(ARGV[budgetOffset + 3]) or 0
  local budgetTtl = tonumber(ARGV[budgetOffset + 4])
...
local offset = 9 + ((selected - 1) * 4)
local id = ARGV[offset + 1]
local keyOffset = 1 + ((selected - 1) * 3)
...
if affinityIndex > 0 then redis.call("SET", affinity, id, "EX", ttl) end
...
if status == 429 then
  ...
  keyIndex = keyIndex + 1
  redis.call("SET", KEYS[keyIndex], "429", "EX", retryAfter)
  keyIndex = keyIndex + 1
elseif status >= 500 then
  redis.call("SET", KEYS[keyIndex], tostring(status), "EX", retryAfter)
  keyIndex = keyIndex + 1
end
if ARGV[4] == "1" then
  local reservedRaw = redis.call("GET", KEYS[keyIndex + 1])
  if reservedRaw then
    local reserved = tonumber(reservedRaw) or 0
    local current = tonumber(redis.call("GET", KEYS[keyIndex])) or 0
    local actual = tonumber(ARGV[5]) or 0
    redis.call("SET", KEYS[keyIndex], tostring(math.max(0, current - reserved + actual)), "EX", tonumber(ARGV[6]) or 60)
...
local reservedRaw = redis.call("GET", KEYS[2])
if not reservedRaw then return {"already-settled"} end
local reserved = tonumber(reservedRaw) or 0
...
export class RedisRoutingStateStore {
  ...
  private credentialScope(providerId: string, credentialId: string) {
    return `${currentWorkspaceId()}:${providerId}:${credentialId}`
  }
  ...
  async reserve(input: ReserveInput) {
    ...
    for (const credential of credentials) {
      const credentialScope = this.credentialScope(input.providerId, credential.id)
    ...
    if (input.budget) {
      ...
      args[8] = keys.length
      args.push(input.budget.limitMicros, input.budget.reservationMicros, input.budget.spentMicros, input.budget.ttlSeconds)
    }
  ...
  async release(input: ReleaseInput) {
    const scope = this.credentialScope(input.providerId, input.credentialId)
    const keys = [`${this.prefix}:inflight:${scope}`]
    const needsCooldown = input.status === 429 || input.status >= 500
    if (input.status === 429) keys.push(`${this.prefix}:rpm:${scope}`)
    if (needsCooldown) keys.push(`${this.prefix}:cooldown:${scope}`)
    const args: Array<string | number> = [
      input.status,
      needsCooldown ? positiveInteger(input.retryAfterSeconds, 5) : 0,
      input.leaseId,
      input.budget ? 1 : 0,
