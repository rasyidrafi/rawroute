...
export async function listMappedCodexAccounts(provider: Provider, workspaceId: string) {
  ...
  if (accounts.some((entry) => entry.credentialKind === "codex-oauth")) {
    ...
    accounts = (await listProviderApiKeys(provider.id)).filter((entry) => entry.credentialKind === "codex-cli-proxy")
  }
  const files = await listCliProxyCodexAuthFiles()
  const byName = new Map(files.map((file) => [file.name, file]))
  return accounts.map((account) => {
    const file = byName.get(mappedFileName(account, workspaceId))
    if (!file) return { ...account, cliProxyStatus: "missing", cliProxyStatusMessage: "Credential mapping is missing in CLIProxy.", enabled: false }
    return {
      ...account,
      accountId: file.accountId || account.accountId,
      email: file.email || account.email,
...
export async function registerCliProxyCodexAccount(provider: Provider, input: { name?: string; authFile: CliProxyAuthFile }) {
  ...
  return upsertProviderApiKey(provider.id, {
    name: input.name?.trim() || file.email || file.accountId || "Codex account",
    key: "",
    credentialKind: "codex-cli-proxy",
...
export async function startCliProxyCodexLogin() {
  ...
  if (!response.ok || !url) throw new Error(`CLIProxy Codex login could not be started (${response.status}).`)
  return { url, existingAuthFiles: before.map((file) => file.name) }
}

export async function completeCliProxyCodexLogin(existingAuthFiles: string[], workspaceId: string, name?: string) {
  const files = await listCliProxyCodexAuthFiles()
  const before = new Set(existingAuthFiles)
  const file = files.find((entry) => !before.has(entry.name))
  if (!file) return undefined
  const response = await cliproxyManagement("/v0/management/auth-files/fields", {
    method: "PATCH",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ name: file.name, prefix: codexWorkspacePrefix(workspaceId) }),
  })
